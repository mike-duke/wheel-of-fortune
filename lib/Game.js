class Game {
  ...
  startGame(playerArray) {
    ...
    this.startRound();
    domUpdates.displayInstructions(`${(this.playerArray[0].name).toUpperCase()}, Spin the Wheel!`);
  ...
  checkLetterGuess(event) {
    const correctLetters = domUpdates.checkLetterGuess(event);
    if(correctLetters.length) {
      this.playerArray.forEach((player) => {
        if (player.isTurn) {
          player.roundScore += (this.wheel.currentValue * correctLetters.length);
          domUpdates.updateRoundScore(player);
        }
      });
  ...
  spinWheel() {
    ...
    if (this.wheel.currentValue === 'BANKRUPT') {
      const currentPlayer = this.playerArray[this.currentTurnIndex]
      currentPlayer.roundScore = 0;
      domUpdates.updateRoundScore(currentPlayer);
      this.endTurn();
    } else if (this.wheel.currentValue === 'LOSE A TURN') {
      this.endTurn();
    }
  }
  ...
  endTurn() {
    ...
    domUpdates.displayInstructions(`${(this.playerArray[this.currentTurnIndex].name).toUpperCase()}, Spin the Wheel!`);
    domUpdates.activatePlayer(this.playerArray[this.currentTurnIndex].playerNumber);

    if (typeof this.wheel.currentValue !== 'string') {
  ...
  endRound() {
    if(this.currentRound < 4) {
      domUpdates.displayRoundSummary();
    }
  }
}
